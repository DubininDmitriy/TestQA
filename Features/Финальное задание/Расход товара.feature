...
Переменные:
	СсылкаДляТестирования = "e1cib/data/Документ.РасходТовара?ref=4b63e2b754934758af697737598b9701"
...
Сценарий: Подготовка документа для тестирования проведения
* Подготовка документа (создание)
	Когда подготовка документа для проверки "СсылкаДляТестирования"

Сценарий: Печать расходной накладной
* Открытие тестируемого документа
	Дано Я открываю навигационную ссылку "СсылкаДляТестирования"
	И я получаю состояние объекта по навигационной ссылке "СсылкаДляТестирования" (расширение)
...
* Печать и проверка
	...
	Тогда табличный документ 'SpreadsheetDocument' равен по шаблону:
		...
		| 'Склад'         | 'Большой'                                                  | ''           | ''       |
		| 'Сумма'         | '12 350 рублей (Двенадцать тысяч триста пятьдесят рублей)' | ''           | ''       |
		| 'Вес'           | ''                                                         | ''           | ''       |
		| ''              | ''                                                         | ''           | ''       |
		| 'Товар'         | 'Цена'                                                     | 'Количество' | 'Сумма'  |
		| 'Сапоги'        | '4 000'                                                    | '3'          | '12 000' |
		| 'Доставка'      | '350'                                                      | '1'          | '350'    |
...
* Подготовка документа (открытие + отмена проведения)
	Дано Я открываю навигационную ссылку "СсылкаДляТестирования"
	И я получаю состояние объекта по навигационной ссылке "СсылкаДляТестирования" (расширение)
	Если 'Контекст.Проведен' Тогда
...
Сценарий: Очистка движений документов и переменных
	И я закрываю все окна клиентского приложения
	И я запоминаю навигационную ссылку "СсылкаДляТестирования" для удаления (расширение)
